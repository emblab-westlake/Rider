...
# Version: 1.8.0 (Updated: Added Quality Filter for Seq Output)
# Author: Gaoyang Luo
...
# def process_m8_file(m8_file_path, output_dir, seq_file_path, prob_threshold=50, top_n=1):
#     """
#     Process a .m8 file. 
#     Generates output files:
#     1. final_result_taxonomy_id.txt (Only removes Rider_)
#     2. final_result_taxonomy_id_with_seq.txt (High Quality: Removes Rider_, suffix, and _1 for lookup)
#     3. final_result_taxonomy_id_with_seq_low_quality.txt (Low Quality: Same format as above)
#     """
#     col_names = [
...
#         df = df.dropna(subset=['bits', 'qtmscore']) # Drop rows where critical scores are NaN
...
#         # 5. Retrieve Best Target info (including qstart/qend)
...
#         # 6. Filter based on Average bits
...
#     if results:
#         # --- Output 1: Original File (Only remove Rider_) ---
...
#                 # Logic for Output 1: Only remove "Rider_" prefix
#                 # Example: Rider_SRR..._1;1-33 -> SRR..._1;1-33
...
#         # --- Output 2 & 3: New File with Sequence and Coordinates (With Quality Filter) ---
#         # Load Raw Sequences
...
#         with open(output_target_path, 'w') as f_tgt, \
#              open(output_rest_path, 'w') as f_rst:
            
#             for query, target, bits, qtm, ttm, lddt, evalue, qstart, qend in results:
#                 # Example query: Rider_SRR5947895_k99_11266_2_1;1-33
                
#                 # 1. Remove "Rider_" prefix
#                 temp_query = query.replace("Rider_", "") # -> SRR5947895_k99_11266_2_1;1-33
                
#                 # 2. Parse ID and Coordinates
#                 start_pos, end_pos = "NA", "NA"
#                 id_part_full = temp_query 
                
#                 if ";" in temp_query:
#                     id_part_full, coords = temp_query.split(";", 1) 
#                     # id_part_full -> SRR5947895_k99_11266_2_1
                    
#                     # Parse Coordinates from header
#                     if "-" in coords:
#                         try:
...
#                 # 3. Clean ID: Remove the last underscore and number (e.g., _1)
#                 # SRR5947895_k99_11266_2_1 -> SRR5947895_k99_11266_2
#                 if "_" in id_part_full:
#                     clean_id = id_part_full.rsplit("_", 1)[0]
#                 else:
#                     clean_id = id_part_full

#                 # 4. Match Sequence using clean_id (The one without _1)
#                 sequence = seq_dict.get(clean_id, "Sequence_Not_Found")

#                 # --- 5. Quality Filtering Logic ---
#                 is_high_quality = False
                
#                 try:
#                     # Convert parsed coordinates to float/int for calculation
#                     # Note: bits is already a float from results tuple
#                     s_pos = float(start_pos)
#                     e_pos = float(end_pos)
#                     length = abs(s_pos - e_pos)
#                     score = bits

#                     # Condition 1: Length > 100 and Score > 50
#                     if length >= 100 and score >= 50:
#                         is_high_quality = True
                    
#                     # Condition 2: Length between 80 and 100, Score > 60
#                     elif 80 <= length <= 99 and score >= 60:
#                         is_high_quality = True
                        
#                 except ValueError:
#                     # If start_pos or end_pos are "NA", calculation fails -> Low Quality
#                     is_high_quality = False
...
# def process_fixed_prob_out_folders(input_root_dir, alignment_type=1, n=1, prob_threshold=50):
#     """
...
#                 # Construct Raw Sequence TXT file path
#                 # Path: intermediate_dir/{subdir}_Rider_predicted_results.txt
