...
# Run XGBoost evaluation on multiple test files
...
for i in /path/to/test_set/tokenID/*.pt; do
  ...
  out_path="${out_dir}/evaluation_result_100_10000_xgb_${idx}.npz"

  CUDA_VISIBLE_DEVICES=1 python /path/to/xgb_benchmark.py \
    --test_data "$i" \
    --model_path "/path/to/xgb_model_extra_3negs.json" \
    --save_path "$out_path"
done